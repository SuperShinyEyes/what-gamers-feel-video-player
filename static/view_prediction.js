...
function getCurrentTableRowIndex(table) {
  ...
  for (let i = 1, row; row = table.rows[i]; i++) {
    let diff = Number(currentTime - row.cells[3].innerHTML);
    if (diff >= 0) {
...
function _colorTableRow(table) {
  ...
  const emotion = table.rows[currentRowIndex].cells[1].innerText;
//   if (emotion.length == 0) { 
//     table.rows[currentRowIndex].className = 'no-pred';
//     return;
//   }
  switch (emotion) {
    case 'cuteness' :
...
function getTimelineDataTable(granularity){
    ...
    switch (granularity) {
        ...
        case '3':
            ...
            break;
        default:
            dataTable.addColumn({ type: 'string', id: 'empty' });;
            break;
    }
...
function getLabelColorDict(granularity) {
    switch (granularity) {
        case '4':
            return {
                'cuteness': '#f032e6',
...
function getLabelColorAsColumnIndex(granularity) {
    switch (granularity) {
        case '4':
            return 1;
        case '3':
...
function drawChart() {
  ...
  // Reference: https://developers.google.com/chart/interactive/docs/gallery/timeline
    ...
    const dataTable = getTimelineDataTable(granularity);
    ...
    */
    const bar_colors = [];
    const label_color_dict = getLabelColorDict(granularity);
    const labelColumnIndex = getLabelColorAsColumnIndex(granularity);
    ...
    for (let i = 0, element; element = annotatorIdsAsHTMLElement[i]; i++) {
        ...
        for (let j = 1, row; row = table.rows[j]; j++) {
            let start_sec = parseFloat(row.cells[3].innerHTML);
            let end_sec = parseFloat(row.cells[4].innerHTML);
        
            // let label = row.cells[labelColumnIndex].innerHTML;
            let label = row.cells[1].innerHTML;
